...
class OpenEyeHybridExecutor(ExecutorBase):
    """For the execution of OpenEye Hybrid and FRED docking which optimizes enrichment"""
    ...
    def execute(self, command: str, arguments: list, check=True, location=None):
        ...
        if command not in [EE.HYBRID, EE.FRED]:
            raise ValueError("Parameter command must be a dictionary of the internal OpenEye Hybrid/FRED executable list.")
    ...
    def is_available(self, command: str):
        # check, whether a proper executable is provided
        if command not in [EE.HYBRID, EE.FRED]:
            raise ValueError("Parameter command must be a dictionary of the internal OpenEye Hybrid/FRED executable list.")
        try:
            ...
            if (EE.OE_HYBRID_HELP_IDENTIFICATION_STRING in result.stderr) or (EE.OE_FRED_HELP_IDENTIFICATION_STRING in result.stderr):
                return True
            return False
